...
async def create_account(account_in: AccountCreate, db: AsyncSession = Depends(get_db)):
    ...
    """
    await logger.ainfo("create_account_requested", name=account_in.company_name, workspace_id=str(account_in.workspace_id))

    # 1. Verify workspace exists
    ws_result = await db.execute(select(WorkspaceModel).where(WorkspaceModel.id == account_in.workspace_id))
    ...
    if account_in.assigned_to:
        user_result = await db.execute(select(UserModel).where(UserModel.id == account_in.assigned_to))
    ...
    try:
        ...
        await logger.aerror("nats_publish_account_created_failed", account_id=str(new_account.id), error=str(e))
...
async def list_accounts(
    workspace_id: UUID = Query(..., description="Filter accounts by Workspace UUID (mandatory)"),
    assigned_to: Optional[UUID] = Query(None, description="Filter accounts by assigned AM UUID"),
    db: AsyncSession = Depends(get_db)
):
    """
    Lists accounts under a workspace, optionally filtered by assigned Account Manager.
    """
    stmt = select(AccountModel).where(AccountModel.workspace_id == workspace_id, AccountModel.deleted_at.is_(None))
    if assigned_to:
...
async def get_account(account_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(AccountModel).where(AccountModel.id == account_id, AccountModel.deleted_at.is_(None))
    )
...
async def list_account_contacts(account_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    acc_res = await db.execute(select(AccountModel).where(AccountModel.id == account_id, AccountModel.deleted_at.is_(None)))
    if not acc_res.scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Account not found")

    result = await db.execute(
        select(ContactModel).where(ContactModel.account_id == account_id, ContactModel.deleted_at.is_(None))
    )
    return result.scalars().all()

@router.post("/{account_id}/contacts", response_model=ContactResponse, status_code=status.HTTP_201_CREATED)
...
async def list_account_notes(
    account_id: UUID,
    user_id: UUID = Query(..., description="Owner User ID of the private notes (only own notes returned)"),
    db: AsyncSession = Depends(get_db)
):
    """
...
@router.post("/{account_id}/notes", response_model=NoteResponse, status_code=status.HTTP_201_CREATED)
async def add_account_note(
    ...
    user_res = await db.execute(
        select(UserModel).where(UserModel.id == note_in.user_id, UserModel.workspace_id == account.workspace_id)
    )
    if not user_res.scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Author User not found in this workspace")
