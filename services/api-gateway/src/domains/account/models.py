...
from sqlalchemy import Column, String, Integer, DateTime, Boolean, ForeignKey, Numeric, Text, text
...
class AccountModel(Base):
    ...
    completeness_score = Column(Integer, nullable=False, server_default="0", default=0)  # 0-100
    status = Column(String, nullable=False, server_default="active", default="active")  # active, inactive, archived
    assigned_to = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    source = Column(String, nullable=False, server_default="manual", default="manual")  # discovery, manual
    ...
    # Relationships
    contacts = relationship("ContactModel", back_populates="account", cascade="all, delete-orphan")
    intelligence = relationship("AccountIntelligenceModel", back_populates="account", cascade="all, delete-orphan")
    news = relationship("AccountNewsModel", back_populates="account", cascade="all, delete-orphan")
    notes = relationship("AccountNoteModel", back_populates="account", cascade="all, delete-orphan")
    embeddings = relationship("AccountEmbeddingModel", back_populates="account", cascade="all, delete-orphan")
...
class ContactModel(Base):
    ...
    seniority = Column(String, nullable=True)  # c_level, vp, director, manager, individual
    buying_role = Column(String, nullable=True)  # decision_maker, influencer, evaluator, procurement
...
class AccountIntelligenceModel(Base):
    ...
    intel_type = Column(String, nullable=False)  # summary, news, industry, technology, financial
...
class AccountNewsModel(Base):
    ...
    signal_type = Column(String, nullable=True)  # e.g., expansion, leadership, financial, technology
...
class AccountNoteModel(Base):
    ...
    workspace_id = Column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False)
    account_id = Column(UUID(as_uuid=True), ForeignKey("accounts.id", ondelete="CASCADE"), nullable=False)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)  # Private to this user
...
class AccountEmbeddingModel(Base):
    ...
    source_record_id = Column(UUID(as_uuid=True), nullable=False)  # links to source contact/news/intel record
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), default=func.now())
